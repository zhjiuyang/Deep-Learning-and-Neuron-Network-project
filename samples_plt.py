...
def plot_samples(noise_vals, noisy_input_test, denoised_images, pure_test, nu_samples=4, img_height=64, img_width=64):
    ...
    for i in range(0, nu_samples):
        ...
        axes[i][0].set_ylabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image, pure_image)))
        axes[i][1].imshow(noisy_image, pyplot.cm.gray)
        axes[i][1].set_ylabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image, noisy_image)))
        axes[i][2].imshow(denoised_image, pyplot.cm.gray)
        axes[i][2].set_ylabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image, denoised_image)))
        axes[i][3].imshow(bm3d_denoised, pyplot.cm.gray)
        axes[i][3].set_ylabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image * 255.0, bm3d_denoised)))
        axes[i][4].imshow(nl_denoised, pyplot.cm.gray)
        axes[i][4].set_ylabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image * 255.0, nl_denoised)))
...
def save_samples(noise_vals, noisy_input_test, denoised_images, pure_test, img_height=64, img_width=64):
    ...
    for i in range(0, len(noisy_input_test)):
        ...
        axes[0].set_xlabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image, pure_image)))
        axes[1].imshow(noisy_image, pyplot.cm.gray)
        axes[1].set_xlabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image, noisy_image)))
        axes[2].imshow(denoised_image, pyplot.cm.gray)
        axes[2].set_xlabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image, denoised_image)))
        axes[3].imshow(bm3d_denoised, pyplot.cm.gray)
        axes[3].set_xlabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image * 255.0, bm3d_denoised)))
        axes[4].imshow(nl_denoised, pyplot.cm.gray)
        axes[4].set_xlabel("SSIM: {:.5f}".format(measure1.get_image_ssim(pure_image * 255.0, nl_denoised)))
